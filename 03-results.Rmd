...
sd_mice <- survey_data %>%
  transmute(tranche, likeliness, likeliness_f,
            autos = as_factor(autos), 
            age = fct_relevel(age, "25-44"), 
            size_cat = fct_relevel(size_cat, "2"), 
            income, 
            frequency_cat = fct_relevel(frequency_cat, "One day or less frequently")) %>%
  mice(m = n_imputations, method = "cart", printFlag = FALSE)
```

We generated `r n_imputations` multiple imputed complete datasets with the
...
equations using classification and regression tree methods encoded in the `mice`
package for R [@mice2011; @R-base]. Each imputed dataset contains 130 completed
observations.
...
simulation; we therefore pool the $p$-values from the `r n_imputations` tests
using a methodology proposed by @LichtThesis.
...
tibble(
  ...
  kbl(caption = "Fisher Exact p-value with Imputation", booktabs = TRUE, 
      digits = 4) %>%
  kable_styling()
```


```{r mi_models, eval=FALSE}
# multivariate regression on the imputed datasets: not used
mi_models <- with(sd_mice, lm(likeliness ~ age + size_cat + frequency_cat))
mi_pooled <- pool(mi_models)
modelsummary(mi_pooled, statistic_vertical = FALSE, stars = TRUE)
```
